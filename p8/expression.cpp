...
Expression::Expression(Operator_type type, Expression *LHS, Expression *RHS)
{
    ...
    m_RHS = RHS;
    m_Variable = NULL;
    if(type == OR || type == AND || type == GREATER_THAN || type == GREATER_THAN_EQUAL || type == LESS_THAN_EQUAL || type == LESS_THAN || type == EQUAL || type == NOT_EQUAL || type == TOUCHES || type == NEAR){
        m_gType = INT;
    }
...
int Expression::evalint()
{
    ...
    if(m_kind == "VARIABLE")
        ...
        switch (m_oType) {
            ...
            case NEAR:
                return m_LHS->m_Variable->get_symbol()->getgameobjectValue()->near(m_RHS->m_Variable->get_symbol()->getgameobjectValue());
            case TOUCHES:
                return m_LHS->m_Variable->get_symbol()->getgameobjectValue()->touches(m_RHS->m_Variable->get_symbol()->getgameobjectValue());
            default:
                return m_iValue;
        }
    }
